...
const GRAVITY = 0.5; // Acceleration in px/s
let simSpeed = 1.5;
...
function configDidUpdate() {
	...
	if (skyLightingSelector() === SKY_LIGHT_NONE) {
		appNodes.canvasContainer.style.backgroundColor = 'rgba(0, 0, 0, 0.42)';
	}
...
function randomShellName() {
	return Math.random() < 0.25 ? 'Crysanthemum' : shellNames[(Math.random() * (shellNames.length - 1) + 1) | 0 ];
}
